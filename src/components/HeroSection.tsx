...
const HeroSection = ({
  ...
  showCtaText = true,
  show3DLogo = true,
  noBlackOverlay = false,
}: HeroSectionProps = {}) => {
  const [is3DLoading, setIs3DLoading] = useState(true);

  // Fallback timeout to hide loading state after 10 seconds
  useEffect(() => {
        ...
        <div className="relative z-10 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col-reverse lg:grid grid-cols-2 items-center gap-5 min-h-[85vh] pt-16">
            ...
            <div className="flex-1 flex flex-col justify-center text-left animate-fade-in">
              {/* Main Headline */}
              <h1 className="text-[2.7rem] md:text-7xl font-bold text-white mb-6 !leading-tight animate-slide-up">
                {mainHeadline}{" "}
              ...
              {/* Sub-headline */}
              <h2
                className="text-2xl text-white mb-6 animate-slide-up"
            ...
            {show3DLogo && (
              <div className="flex-1 lg:max-w-lg xl:max-w-xl h-[400px] lg:h-[600px] flex items-center justify-center translate-y-24 sm:translate-y-0 relative w-full min-w-[300px]">
                {/* Loading Overlay */}
                {/* {is3DLoading && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center z-20 bg-black/50 backdrop-blur-sm">
                    <div className="relative">
                      <div className="w-20 h-20 border-4 border-[#ffd63c]/20 border-t-[#ffd63c] rounded-full animate-spin"></div>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="w-10 h-10 bg-[#ffd63c] rounded-lg opacity-60"></div>
                      </div>
                    </div>
                    <p className="text-[#ffd63c] text-lg mt-6 font-medium">
                      Loading 3D Model...
                    </p>
                  </div>
                )} */}
                ...
                <div
                  className={`w-full h-full transition-opacity duration-500
                    `}
                  //    ${
                  //   is3DLoading ? "opacity-0" : "opacity-100"
                  // }
                >
                  <Logo3D
        ...
        <div className="relative z-10 w-full py-[2rem] wrapper text-center bg-gradient-to-b from-transparent to-black">
          <h2 className="text2">
            {(() => {
              ...
              const wordsToHighlight = bottomHighlightedWord
                .split(",")
                .map((w) => w.trim());

              // Create a regex pattern for all words to highlight
              const pattern = new RegExp(
                `(${wordsToHighlight.join("|")})`,
                "gi"
              );
